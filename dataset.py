...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str,
                 train: bool = True, sp_model_prefix: str = None,
                 vocab_size: int = 2000, normalization_rule_name: str = 'nmt_nfkc_cf',
                 model_type: str = 'bpe', max_length: int = 128, split=False):
        """
        Dataset with texts, supporting BPE tokenizer
        :param data_file: file containing texts
        ...
        if not os.path.isfile(sp_model_prefix + '.model'):
            ...
            SentencePieceTrainer.train(
                ...
                pad_id=0, bos_id=1, eos_id=2, unk_id=3, minloglevel=1
            )
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        padded[:min(len(indices), self.max_length)] = torch.tensor(indices)[:min(len(indices), self.max_length)]
